...
static int free_pipe_list(struct pipe *head, int indent)
{
	...
	for (pi=head; pi; pi=next) {
		debug("%s pipe reserved mode %d\n", ind, pi->r_mode);
		rcode = free_pipe(pi, indent);
		debug("%s pipe followup code %d\n", ind, pi->followup);
		next=pi->next;
		pi->next=NULL;
		free(pi);
	}
		...
		if (strcmp(dest->data, r->literal) == 0) {
			...
			if (r->flag & FLAG_START) {
				struct p_context *new = 
						xmalloc(sizeof(struct p_context));
				debug("push stack\n");
		...
		if (m == 0 || ((m == 1 || m == 2) && dest->quote)) {
			...
			case '|':
				...
				if (next=='|') {
					...
					/* we could pick up a file descriptor
					 * choice here with redirect_opt_num(),
					 * but bash doesn't do it.
			...
			default:
				syntax();
				/* this is really an internal logic error */
				return 1;
			}
		}
	}
...
static int parse_stream_outer(struct in_str *inp, int flag)
{
	...
	debug(" ### is_str: *p: %s, __promptme: %d, promptmode: %d ###\n",
		inp->p, inp->__promptme, inp->promptmode);
	...
	do {
		...
		if (rcode != 1 && ctx.old_flag == 0) {
			...
			debug("run_list return: %d\n", code);
			if (code == -2) {
				b_free(&temp);
				code = 0;
				if (inp->peek == file_peek) {
					printf("exit not allowed from"
						" main input shell.\n");
					continue;
				}
				break;
